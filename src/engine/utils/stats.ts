...
if (process.env.NODE_ENV !== 'production') {
    Stats = function() {
        ...
        var fpsPanel = addPanel(Panel('FPS', '#0ff', '#002'));
        var msPanel = addPanel(Panel('MS', '#0f0', '#020'));

        // @ts-ignore
        if (self.performance && self.performance.memory) {

            var memPanel = addPanel(Panel('MB', '#f08', '#201'));
    ...
    function Panel(name: string, fg: string, bg: string) {
