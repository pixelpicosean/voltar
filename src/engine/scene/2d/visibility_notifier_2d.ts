...
export class VisibilityNotifier2D extends Node2D {
    ...
    viewports: Set<Viewport> = new Set;
     ...
     */
    _enter_viewport(viewport: Viewport) {
     ...
     * @param {Viewport} viewport
     */
    _exit_viewport(viewport: Viewport) {
...
export class VisibilityEnabler2D extends VisibilityNotifier2D {
    ...
    nodes: Map<Node, any> = new Map;

    /**
     * @param {number} p_enabler
     * @param {boolean} p_enable
     */
    set_enabler(p_enabler: number, p_enable: boolean) {
     ...
     */
    _change_node_state(p_node: Node, p_enabled: boolean) {
        ...
        if (this.enabler[ENABLER_PAUSE_ANIMATIONS]) {
            if (p_node.class === 'AnimationPlayer') {
                (p_node as AnimationPlayer).playback_active = p_enabled;
            }
        }
        ...
        if (this.enabler[ENABLER_PAUSE_ANIMATED_SPRITES]) {
            if (p_node.class === 'AnimatedSprite') {
                if (p_enabled) {
                    (p_node as AnimationPlayer).play();
                } else {
                    (p_node as AnimationPlayer).stop();
                }
            }
        }
        ...
        if (this.enabler[ENABLER_PAUSE_PARTICLES]) {
            if (p_node.class === 'CPUParticle2D') {
                (p_node as CPUParticles2D).emitting = p_enabled;
            }
        }
    }
