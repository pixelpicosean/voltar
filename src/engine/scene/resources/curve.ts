 ...
 * @param {number} end
 */
function _bezier_interp(t: number, start: number, control_1: number, control_2: number, end: number) {
    const omt = (1.0 - t);
    const omt2 = omt * omt;
    const omt3 = omt2 * omt;
    const t2 = t * t;
 ...
 * @param {Vector2} end
 */
function _bezier_interp_vec(t: number, start: Vector2, control_1: Vector2, control_2: Vector2, end: Vector2) {
    const omt = (1.0 - t);
    const omt2 = omt * omt;
    const omt3 = omt2 * omt;
    const t2 = t * t;
...
class C_Point {
    ...
    static create(
        ...
        p_left: number = 0,
        p_right: number = 0,
        p_left_mode: TangentMode = TangentMode.FREE,
        p_right_mode: TangentMode = TangentMode.FREE
    ) {
...
export class Curve extends VObject {
     ...
     * @param {number} pos
     */
    set_point_value(p_index: number, pos: number) {
     ...
     * @param {number} offset
     */
    set_point_offset(p_index: number, offset: number) {
     ...
     * @param {number} p_index
     */
    get_point_position(p_index: number) {
     ...
     */
    interpolate_local_nocheck(index: number, local_offset: number) {
     ...
     */
    set_point_left_tangent(i: number, tangent: number) {
     ...
     * @param {number} tangent
     */
    set_point_right_tangent(i: number, tangent: number) {
     ...
     * @param {number} p_mode
     */
    set_point_left_mode(i: number, p_mode: number) {
     ...
     * @param {number} p_mode
     */
    set_point_right_mode(i: number, p_mode: number) {
     ...
     * @param {number} i
     */
    get_point_left_tangent(i: number) {
        return this._points[i].left_tangent;
    }
    /**
     * @param {number} i
     */
    get_point_right_tangent(i: number) {
        return this._points[i].right_tangent;
    }
    /**
     * @param {number} i
     */
    get_point_left_mode(i: number) {
        return this._points[i].left_mode;
    }
    /**
     * @param {number} i
     */
    get_point_right_mode(i: number) {
     ...
     */
    set_bake_resolution(p_resolution: number) {
     ...
     * @param {number} offset
     */
    interpolate_baked(offset: number) {
...
export class Curve2D extends VObject {
     ...
     */
    add_point(p_pos: Vector2, p_in: Vector2, p_out: Vector2, p_atpos: number = -1) {
        const n = new Point();
        n.pos.copy(p_pos);
        n.in.copy(p_in);
        n.out.copy(p_out);
     ...
     * @param {Vector2} p_pos
     */
    set_point_position(p_index: number, p_pos: Vector2) {
     ...
     * @param {Vector2} p_in
     */
    set_point_in(p_index: number, p_in: Vector2) {
     ...
     * @param {Vector2} p_out
     */
    set_point_out(p_index: number, p_out: Vector2) {
     ...
     */
    interpolate(p_index: number, p_offset: number) {
     ...
     */
    interpolate_baked(p_offset: number, p_cubic: boolean = false) {
        if (this.baked_cache_dirty) {
            this._bake();
        }
     ...
     */
    get_closest_point(p_to_point: Vector2) {
        // Brute force
     ...
     * @param {Vector2} p_to_point
     */
    get_closest_offset(p_to_point: Vector2) {
        if (this.baked_cache_dirty) {
            this._bake();
        }
     ...
     */
    tessellate(p_max_stages: number = 5, p_tolerance: number = 4) {
        /** @type {Vector2[]} */
        const tess: Vector2[] = [];
        ...
        const midpoints: Map<number, Vector2>[] = new Array(len);
     ...
     * @param {number} p_tol
     */
    _bake_segment2d(r_bake: Map<number, Vector2>, p_begin: number, p_end: number, p_a: Vector2, p_out: Vector2, p_b: Vector2, p_in: Vector2, p_depth: number, p_max_depth: number, p_tol: number) {
