...
export class BaseButton extends Control {
    ...
    _gui_input_(p_event: InputEvent) {
        ...
        const mouse_button = (p_event.class === 'InputEventMouseButton') ? (p_event as InputEventMouseButton) : null;
        ...
        if (p_event.class === 'InputEventMouseMotion') {
            const mouse_motion: InputEventMouseMotion = p_event as InputEventMouseMotion;
            if (this.status.press_attempt) {
     ...
     * @param {InputEvent} p_event
     */
    on_action_event(p_event: InputEvent) {
