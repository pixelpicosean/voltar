export function earcut(data: number[], hole_indices?: number[], dim?: number) {
    dim = dim || 2;

    var hasHoles = hole_indices && hole_indices.length,
        outerLen = hasHoles ? hole_indices[0] * dim : data.length,
...
export function deviation(data: number[], holeIndices: number[], dim: number, triangles: number[]): number {
...
export function flatten(data: number[][][]): { vertices: number[], holes: number[], dimensions: number } {
    var dim = data[0][0].length,
        result = { vertices: [] as number[], holes: [] as number[], dimensions: dim },
        holeIndex = 0;
