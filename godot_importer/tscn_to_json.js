...
const make_node_block_list = (blocks) => {
    return blocks
 ...
 */
function normalize_resource(obj, meta, __final_meta__, owner) {
    if (!owner) owner = obj;
    ...
    for (let k in obj) {
        ...
        if (_.isString(value)) {
            if (value.indexOf('ExtResource') >= 0) {
                ...
                obj[k] = `@ext#${key}`;
                __final_meta__.ext[key] = resource_normalizers[res.type](res, meta, obj);
            } else if (value.indexOf('SubResource') >= 0) {
                const key = get_function_params(value)[0];
                let res = meta.sub_resource[key];
                obj[k] = `@sub#${key}`;
                if (resource_normalizers[res.type]) {
                    __final_meta__.sub[key] = resource_normalizers[res.type](res, meta, obj);
                } else {
        ...
        if (_.isObject(value)) {
            normalize_resource(value, meta, __final_meta__, owner);
        }
    }
    ...
    if (obj.children) {
        obj.children.forEach(child => {
            normalize_resource(child, meta, __final_meta__, owner);
        });
    }
}
