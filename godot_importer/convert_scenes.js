...
const { convert_block } = require('./converter/convert_block');
 ...
 * @param {any[]} blocks
 */
function make_node_block_list(blocks) {
    return blocks
 ...
 * @param {any[]} blocks
 */
function fetch_res(type, blocks) {
    return blocks
        .filter(b => b.key === `${type}_resource`)
        .reduce((hash, b) => {
            hash[b.id] = b;
...
const resource_normalizers = {
    ...
    DynamicFontData: (res) => res,
    DynamicFont: (res) => res,
    // TODO: add bitmap font to preload assets list in `resources.json`
    BitmapFont: (res) => path.basename(res.path, '.fnt'),
    RectangleShape2D: (res) => res,
    CircleShape2D: (res) => res,
...
/**
 * @param {any} obj Node or its object property
 * @param {any} meta meta of the Node
 * @param {any} __final_meta__ final meta of the Node
 * @param {any} [owner] Node
 */
function normalize_resource(obj, meta, __final_meta__, owner, resource_map) {
    if (!owner) owner = obj;
    ...
    for (let k in obj) {
        ...
        if (_.isString(value)) {
            if (value.indexOf('ExtResource') >= 0) {
                ...
                obj[k] = `@ext#${key}`;
                __final_meta__.ext[key] = resource_normalizers[res.type](res, meta, obj, resource_map);
            } else if (value.indexOf('SubResource') >= 0) {
                const key = get_function_params(value)[0];
                let res = meta.sub_resource[key];
                obj[k] = `@sub#${key}`;
                if (resource_normalizers[res.type]) {
                    __final_meta__.sub[key] = resource_normalizers[res.type](res, meta, obj, resource_map);
                } else {
                    __final_meta__.sub[key] = res;
                }
            }
        }

        if (_.isObject(value)) {
            normalize_resource(value, meta, __final_meta__, owner, resource_map);
        }
    }

    const post_action = post_resource_actions[obj.type];
    if (post_action) {
        post_action(obj, meta);
    }

    if (obj.children) {
        obj.children.forEach(child => {
            normalize_resource(child, meta, __final_meta__, owner, resource_map);
        });
    }
}
...
function convert_scene(data, scene_db, resource_map) {
    const scene = construct_scene(data, scene_db);

    // Normalize resources
    scene.__final_meta__ = { ext: {}, sub: {} };
    normalize_resource(scene, scene.__meta__, scene.__final_meta__, null, resource_map);
        ...
        listeners: {
            file: function (root, file_stats, next) {
                if (path.extname(file_stats.name) === '.tscn') {
                    ...
                    // convert_tres(scene_root_url, filename, resource_map);
                } else {
                    next();
                }
            },
